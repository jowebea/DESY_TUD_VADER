/*
  ...
  Mini2-Protokoll (32-Bit Word, Big-Endian):
    Python  -> Arduino (SET):
...
/* ==== VOLLSTÄNDIGE STRUKTUREN – MÜSSEN VOR DEN FUNKTIONEN STEHEN ==== */
struct Mini2Set {
  uint16_t setpoint_raw;     // 0.1 kPa
  uint8_t  mode2;            // Info
  uint8_t  vp1PWM;           // 0..255
  uint8_t  vp2PWM;           // 0..255
  uint16_t ramp_speed_raw16; // 0.01 kPa/s
  uint16_t ramp_end10;       // 0..1023 kPa
};
...
/* ---- FUNKTIONS-PROTOTYPEN (optional, aber sauber) ----------------------- */
...
static bool popWordFromSerial(uint32_t &w);
static void handle_comms_mini2();
static void protect_sensors();
static float readPressure();
static void control_pressure();
static void update_ramp_target_if_active();
...
enum ControlState { STATE_MANUAL=0, STATE_RAMP=1, STATE_AUTOMATIC=2 };
...
void protect_sensors(){
    ...
    if (p21 > 191.0f) { digitalWrite(VS2_PIN, CLOSE); vs2State = CLOSE; P20_online = false; }
    else if (p21 < 170.0f) { digitalWrite(VS2_PIN, OPEN); vs2State = OPEN; P20_online = true; }
}
float readPressure() { return (P20_online) ? mapP20(accurateADC_P20()) : mapP21(accurateADC_P21()); }
...
void control_pressure() {
  ...
  if (fabs(error) <= TOLERANCE) {
    integral_increase = 0; integral_decrease = 0;
...
inline void update_ramp_target_if_active() {
  ...
  if (ramp_speed_kpa_s <= 0.0f || fabs(ramp_end_kpa - ramp_start_kpa) < 1e-6f) {
    target_p = ramp_end_kpa; ctrl_state = STATE_AUTOMATIC; return;
  }
  ...
  if ((ramp_dir > 0 && candidate >= ramp_end_kpa) || (ramp_dir < 0 && candidate <= ramp_end_kpa)) {
    target_p = ramp_end_kpa; ctrl_state = STATE_AUTOMATIC;
  } else {
    target_p = candidate;
  }
}
...
/* --------------------------- Mini2 I/O ------------------------------------ */
static volatile bool status_requested = false;

// Robuster byteweiser RX-Parser (kein blockierendes readBytes)
static bool popWordFromSerial(uint32_t &w) {
  static uint8_t buf[4]; static uint8_t idx = 0;
...
static bool applySetFrame(uint8_t baseAddr, uint32_t word, Mini2Set &state) {
  uint8_t addr; uint32_t payload; splitWord(word, addr, payload);
...
static void makeStatusFrames(uint8_t baseAddr, const Mini2Status &s, uint32_t outWords[3]) {
  outWords[0] = makeWord(baseAddr + 2, ((uint32_t)(s.p20_raw & 0xFFFF) << 10));
  outWords[1] = makeWord(baseAddr + 3, ((uint32_t)(s.p21_raw & 0xFFFF) << 10));
  outWords[2] = makeWord(baseAddr + 4,
              ((uint32_t)(s.mode2 & 0x03) << 24)
...
static void handle_comms_mini2(){
  uint32_t w;
  while (popWordFromSerial(w)) {
    uint8_t a; uint32_t p; splitWord(w, a, p);

    // BASE+6: SEND_STATUS vormerken
    if (a == (uint8_t)(BASE_ADDR + 6)) {
      if (p & 0x1) status_requested = true;
    }

    // Andere Befehle übernehmen
  ...
  if (status_requested) {
    ...
    uint32_t words[3]; makeStatusFrames(BASE_ADDR, s, words);
    uint8_t out[4];
    for (int i=0;i<3;i++){ u32ToBytesBE(words[i], out); Serial.write(out, 4); }
    status_requested = false;
  }
}

/* --------------------------- State Steps ---------------------------------- */
inline void step_manual(){ analogWrite(VP1_PIN, manual_pwmVP1); pwmVP1 = manual_pwmVP1; analogWrite(VP2_PIN, manual_pwmVP2); pwmVP2 = manual_pwmVP2; }
inline void step_ramp(){ update_ramp_target_if_active(); for (int i=0; i<5; ++i){ control_pressure(); } }
inline void step_automatic(){ for (int i=0; i<5; ++i){ control_pressure(); } }

/* --------------------------- Setup / Loop --------------------------------- */
void setup(){
  Serial.begin(115200);
  Serial.setTimeout(5);
  while (!Serial) { ; }  // USB/CDC bereit (wichtig bei Boards mit USB-Serial)
...
void loop(){
  ...
  // Status wird nur auf Anfrage (BASE+6 Bit0) gesendet
}
